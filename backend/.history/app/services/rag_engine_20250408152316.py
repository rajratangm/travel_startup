...
async def handle_query(query):
    ...
    source_plan = await decide_retrieval_plan(question)

    # Step 2: Fetch relevant info based on the plan
    context_parts = []

    if "vector" in source_plan:
        vector_data =  await retrieve_from_vector_db(question)
        context_parts.extend(vector_data)

    if "sql" in source_plan:
        sql_data = await retrieve_from_sql(question)    
    ...
    if "web" in source_plan:
        web_data = await fetch_from_web(question)
        context_parts.extend(web_data)

    # Step 3: Final answer from LLM using aggregated context
    final_response = await generate_response(question, context_parts)
